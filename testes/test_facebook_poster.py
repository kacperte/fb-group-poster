...
class TestFacebookPoster(unittest.TestCase):
    ...
    def test_login_with_invalid_credentials(self):
        login = 'tesasdast@tccccest.com'
        password = 'wrongpassword'
        self.my_class.login = login
        self.my_class.password = password
        self.my_class._login_to_facebook(human_simulation=False)
    ...
    def test_time_pattern_with_argument(self):
        tp = 5
        start_time = time.time()
        self.my_class._time_patterns(tp)
        end_time = time.time()
        self.assertAlmostEqual(end_time - start_time, tp, delta=0.1)

    def test_time_pattern_with_default(self):
        start_time = time.time()
        self.my_class._time_patterns()
        end_time = time.time()
        self.assertAlmostEqual(end_time - start_time, self.my_class.time_pattern, delta=0.1)
